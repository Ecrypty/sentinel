...
def has_florijncoin_conf():
    import config
    import io

    valid_florijncoin_conf = False

    # ensure florijncoin_conf exists & readable
    #
    # if not, print a message stating that Florijncoin Core must be installed and
    # configured, including JSONRPC access in florijncoin.conf
    try:
        f = io.open(config.florijncoin_conf)
        valid_florijncoin_conf = True
    except IOError as e:
        print(e)

    return valid_florijncoin_conf
...
def main():
    ...
    if not has_florijncoin_conf():
        print("FlorijncoinCore must be installed and configured, including JSONRPC access in florijncoin.conf")
        sys.exit(1)
