...
sentinel_cfg = FlorijncoinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.5.0"


def get_florijncoin_conf():
    if sys.platform == 'win32':
        florijncoin_conf = os.path.join(os.getenv('APPDATA'), "FlorijncoinCore/florijncoin.conf")
    else:
        home = os.environ.get('HOME')

        florijncoin_conf = os.path.join(home, ".florijncoincore/florijncoin.conf")
        if sys.platform == 'darwin':
            florijncoin_conf = os.path.join(home, "Library/Application Support/FlorijncoinCore/florijncoin.conf")

    florijncoin_conf = sentinel_cfg.get('florijncoin_conf', florijncoin_conf)

    return florijncoin_conf
...
florijncoin_conf = get_florijncoin_conf()
network = get_network()
