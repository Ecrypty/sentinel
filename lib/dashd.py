"""
florijncoind JSONRPC interface
"""
import sys
import os
...
class FlorijncoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some florijncoind methods
    ...
    @classmethod
    def from_florijncoin_conf(self, florijncoin_dot_conf):
        from florijncoin_config import FlorijncoinConfig
        config_text = FlorijncoinConfig.slurp_config_file(florijncoin_dot_conf)
        creds = FlorijncoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = florijncoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = florijncoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = florijncoinlib.blocks_to_seconds(diff)
