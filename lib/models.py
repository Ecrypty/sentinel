...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, florijncoind):
        golist = florijncoind.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_florijncoind(florijncoind, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_florijncoind(self, florijncoind, rec):
        import decimal
        ...
        # exclude any invalid model data from florijncoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from florijncoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from florijncoind - Florijncoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(florijncoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from florijncoind! %s" % e)
            govobj.vote_delete(florijncoind)
            return (govobj, None)
    ...
    def vote_delete(self, florijncoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(florijncoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, florijncoind, signal, outcome):
        import florijncoinlib
        ...
        output = florijncoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = florijncoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(florijncoind, signal)

    def sync_network_vote(self, florijncoind, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = florijncoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = FLRND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 florijncoin addr, non-multisig
            if not florijncoinlib.is_valid_florijncoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Florijncoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Florijncoin Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (florijncoinlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = FLRND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not florijncoinlib.is_valid_florijncoin_address(addr, config.network):
    ...
    def hash(self):
        import florijncoinlib
        return florijncoinlib.hashit(self.serialise())
