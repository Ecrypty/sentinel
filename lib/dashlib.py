...
def is_valid_florijncoin_address(address, network='mainnet'):
    ...
    florijncoin_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Florijncoin address
        return False

    if (address_version != florijncoin_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('florijncoin.conf')
