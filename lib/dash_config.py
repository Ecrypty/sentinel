...
class FlorijncoinConfig():

    @classmethod
    def slurp_config_file(self, filename):
        # read florijncoin.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is florijncoin.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from florijncoin.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Florijncoin defaults...
        default_port = 9998 if (network == 'mainnet') else 19998

        # use default port for network if not specified in florijncoin.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from florijncoin.conf
