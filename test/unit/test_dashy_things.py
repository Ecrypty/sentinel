...
def test_valid_florijncoin_address():
    from florijncoinlib import is_valid_florijncoin_address

    main = valid_florijncoin_address()
    test = valid_florijncoin_address('testnet')

    assert is_valid_florijncoin_address(main) is True
    assert is_valid_florijncoin_address(main, 'mainnet') is True
    assert is_valid_florijncoin_address(main, 'testnet') is False

    assert is_valid_florijncoin_address(test) is False
    assert is_valid_florijncoin_address(test, 'mainnet') is False
    assert is_valid_florijncoin_address(test, 'testnet') is True


def test_invalid_florijncoin_address():
    from florijncoinlib import is_valid_florijncoin_address

    main = invalid_florijncoin_address()
    test = invalid_florijncoin_address('testnet')

    assert is_valid_florijncoin_address(main) is False
    assert is_valid_florijncoin_address(main, 'mainnet') is False
    assert is_valid_florijncoin_address(main, 'testnet') is False

    assert is_valid_florijncoin_address(test) is False
    assert is_valid_florijncoin_address(test, 'mainnet') is False
    assert is_valid_florijncoin_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from florijncoinlib import elect_mn
...
def test_parse_masternode_status_vin():
    from florijncoinlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % florijncoinlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(florijncoinlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(florijncoinlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(florijncoinlib.blocks_to_seconds(16616)) == 2612035
